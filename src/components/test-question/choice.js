  ...
  return (
    <Grid container>
      ...
      {Object.values(question.answerOptions).map((answerOption) => (
        <Grid container item xs={12} key={answerOption.id} style={(showAnswers && testAnswers[answerOption.id].isCorrect ? correctAnswerStyle : answerStyle)}>
          <Grid item xs={1} style={{marginBlockStart: 'calc(1em - 12px)', textAlign: 'right'}}>
            <Checkbox id={`chk-${answerOption.id}`} checked={Object.prototype.hasOwnProperty.call(answers, answerOption.id)} onChange={selectAnswer(answerOption)} />
          </Grid>
          <Grid item xs={11}>
